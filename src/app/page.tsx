...
async function getDashboardData() {
  const [totalStudents, totalTeachers, totalClasses, announcements] =
    await Promise.all([
  ...
  const schedules = await prisma.schedule.findMany({
    where: { day: new Date().getDay() || 7 }, // 1=Mon...6=Sat
    include: {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="font-headline text-3xl font-bold text-primary-fixed">
                    Rp {collectedK > 0 ? `${collectedK}k` : "0"}
                  </span>
                  <span className="text-xs bg-primary-container px-2 py-0.5 rounded text-on-primary-container">
                    {percentage}%
                  </span>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center">
                <p className="text-xs opacity-80">
                  Pending: Rp {pendingK > 0 ? `${pendingK}k` : "0"}
                </p>
                <a
                  href="/keuangan"
                  className="text-sm font-bold text-primary-fixed hover:underline"
                >
                  Rincian →
                </a>
              </div>
        ...
        <div className="md:col-span-2 bg-surface-container-lowest p-6 rounded-xl card-shadow border border-outline-variant">
          <div className="flex justify-between items-center mb-4">
            ...
            <a href="/jadwal" className="text-primary font-semibold text-sm">
              Lihat Semua
            </a>
          </div>
          <div className="space-y-4">
            {data.schedules.length > 0 ? (
              data.schedules.map((s, i) => (
                <ScheduleItem
                  ...
                  detail={`${s.class.name} • ${s.teacher.user.name}`}
                  active={i === 0}
                />
              ))
            ) : (
        ...
        {/* Quick Links */}
        <div className="md:col-span-2 bg-surface-container-lowest p-6 rounded-xl card-shadow border border-outline-variant">
          <h3 className="font-headline text-xl font-semibold text-on-surface mb-4">
            Menu Cepat
          </h3>
